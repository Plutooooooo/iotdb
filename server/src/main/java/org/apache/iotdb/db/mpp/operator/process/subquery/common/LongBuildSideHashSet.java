...
public class LongBuildSideHashSet extends BuildSideHashSet {

  private LongBigArray values;
  private IntBigArray elementIds;
  private LongBigArray valuesByElementId;
   ...
   */
  public LongBuildSideHashSet(int hashChannel, int estimatedElementCount) {
    ...
    valuesByElementId = new LongBigArray();
  ...
  private int addNewElement(int hashPosition, long value) {
    // record element id
  ...
  protected boolean tryRehash() {
    ...
    for (int elementId = 0; elementId < nextElementId; elementId++) {
      ...
      long value = valuesByElementId.get(elementId);
  ...
  private int getHashPosition(long value, int mask) {
    return Long.hashCode(value) & mask;
  }
